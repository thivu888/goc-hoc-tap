...
export default function Topbar({setChatBoxShow,setUserConnect,socket}) {
  ...
  useEffect(()=>{
   ...
   socket.on('send-notify-seen-client',data=>{
  ...
  return (
    <div className="topbarContainer container-fluid">
      ...
      <NavLink  style={{display:'inline-block',width:'100px'}} to='/' isActive={(match,location)=>{ if(match){ if(match.isExact)setactive('home')}}} activeClassName={'topbarCenter-item-active'} >
        <li className='topbarCenter-item' style={{display:'inline-block',}} >
